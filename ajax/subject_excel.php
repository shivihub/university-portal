            ...
            $object= PHPExcel_IOFactory::load($uploadFilePath);
             foreach($object->getWorksheetIterator() as $worksheet){
                 ...
                 for($row=2; $row<=$rowcount;$row++){
                    ...
                    $credit_points=$worksheet->getCellByColumnAndRow(2,$row)->getValue();
                    $semester=$worksheet->getCellByColumnAndRow(3,$row)->getValue();
                    $query = $db->prepare('INSERT INTO courses_of_ece(course_name,course_code,credit_points,semester) Values (?,?,?,?)');
                    $data = array($course_name,$code,$credit_points,$semester);
